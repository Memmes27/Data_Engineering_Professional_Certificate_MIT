...
def buybook():
    ...
    # Handle existing cookie and new book purchase
    if booksCookie:
        bookIds = booksCookie.split(',')
    ...
    # Add new book if bookId parameter exists
    if bookIdParam:
        for book in books:
            if str(book['id']) == bookIdParam and book not in booksToPurchase:
                booksToPurchase.append(book)
                totalDue += book['price']

    # Create the updated cookie value
    cookieValue = ','.join(str(book['id']) for book in booksToPurchase)
    
    # Create response with template
    response = make_response(render_template('buybook.html', 
                           booksToPurchase=booksToPurchase, 
                           totalDue=totalDue, 
                           username=user))
    
    # Set the cookie
    response.set_cookie('booksToPurchase', cookieValue)
